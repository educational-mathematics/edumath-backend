...
def _norm_media_url(u: str | None) -> str | None:
    if not u: return None
    s = u.strip()
    if s.startswith("http://") or s.startswith("https://"):
        return s
    if s.startswith("/media/") or s.startswith("/static/"):
        return s
    # fallback: asume que es relativo dentro de media
    return "/media/" + s.lstrip("/")

@router.get("/me", response_model=UserOut)
def read_me(current_user: UserModel = Depends(get_current_user)):
    # normaliza on-the-fly sin tocar DB
    current_user.avatar_url = _norm_media_url(current_user.avatar_url) or current_user.avatar_url
    return current_user
...
def upload_avatar(
    ...
    # Guarda SIEMPRE una ruta relativa al backend (/media/...)
